...
void rr( ) //vraćena u red pripravnih, iza dretvi istog prioriteta, ali ispred onih manjeg prioriteta.
{
	int i, indeks;
	dretva *temp;
	temp = (dretva*)malloc( sizeof(dretva));
	...
	for (i = 0; i < MAX_DRETVI; ++i)
	{
			if ( P[i] != NULL && P[i]->prio == P[0]->prio ) //P[0] je aktivna
				indeks = i;
	}
	for (i = 0; i < indeks; ++i)
		P[i] =P[i+1];
		...
		if ( P[0] != NULL )  // P[0] je aktivna dretva
		{
            ...
            if ( P[0]->p == 0 )
                ...
                --zadnje;  // zadnje ne-null mjesto u nizu P
					  ...
					  if ( P[0]!= NULL && P[0]->p != 0 && P[0]->rasp==1 ) // rasp je RR
							rr( );
    }
